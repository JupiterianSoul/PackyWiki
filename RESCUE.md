...
3. Open **Wiklodo** on the phone and leave it on screen.
4. On the computer, open Chrome (or Edge) and go to **`chrome://inspect`**.
   The Wiklodo WebView appears under the phone's name. Click **inspect**.
5. In the DevTools **Console** that opens, paste this and press Enter:

```js
copy(JSON.stringify({
  format: 'wiklodo-save', version: 1, at: Date.now(),
  data: Object.fromEntries([
    ...
    'packywiki.ripDirection', 'packywiki.theme'
...
The paste box refuses anything that is not a Wiklodo save, and it replaces the
