...
export const RELEASES = [
  ...
  },
  {
    ...
    changelog: [
      ...
      { en: 'A tier is now a band rather than a floor: a booster deals cards of the rarity printed on it, always at least one, and never pays out above it. When the subject runs thin the rest come from the most read pages below the tier, which still beats a booster with no tier at all. Before, a thin subject filled the pack from the open pool and an Epic could hold five Commons', fr: 'Un palier est désormais une fourchette et non un simple plancher : un booster donne des cartes de la rareté inscrite dessus, toujours au moins une, et ne paie jamais au-dessus. Quand le sujet s’épuise, le reste vient des pages les plus lues sous le palier, ce qui vaut encore mieux qu’un booster sans palier. Avant, un sujet pauvre remplissait le paquet dans le tas commun et un Épique pouvait contenir cinq Communes' },
