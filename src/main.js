...
import { boosterPrice, sellPriceFor, nextRefreshAt, windowIndexAt, drawCapsFor, nextFreeAt, freeWindowAt, STARTER_PACKS, STARTER_PACK_CARDS, STARTER_COINS, cratePriceAt } from './economy.js';
import { generateShop, formatCountdown, rollCrate, crateReel } from './shop.js';
import { CUSTOM_CARD_RANGE } from './economy.js';
import {
...
const shopPainters = [];
const repaintShop = () => { for (const paint of shopPainters) { try { paint(); } catch { /* a tile that is gone */ } } };
 ...
 */
function buyButton(spec, price, { count = 1, after = null, item = null, label = null } = {}) {
...
function shopTile(item, { free = false } = {}) {
...
const crateBoughtNow = () => store.shopBought(state.profile, CRATE_ID);
const cratePriceNow = () => cratePriceAt(crateBoughtNow());

function buildCrateStall() {
...
function showCrateRoll(winner) {
  ...
  openSheet(t('shopCrate'), (body) => {
    ...
    const finish = () => {
      ...
      again.addEventListener('click', () => { synth.playTap(); sheet.hide(); setTimeout(() => openCrate(null), 250); });
      ...
      done.addEventListener('click', () => { synth.playTap(); sheet.hide(); });
...
function purchase(spec, price, button, count = 1, item = null) {
...
function purchaseBundle(item, button) {
 ...
 */
const releaseTitle = (release) => tx(release.title);
const releaseNumber = (release) => RELEASES.indexOf(release) + 1;
